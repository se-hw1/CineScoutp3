...
const SocialMovieDiscovery = () => {
    ...
    const [submittedRatings, setSubmittedRatings] = useState([]); // Stores submitted ratings
    ...
    const handleSubmitRatings = () => {
        const ratedMovies = Object.keys(ratings).map((movieId) => {
            const movie = mockMovies.find((m) => m.id === parseInt(movieId, 10));
            return { title: movie.title, rating: ratings[movieId] };
        });
        setSubmittedRatings(ratedMovies);
    ...
    // Handle Clear All Movie Nights
    const handleClearAllMovieNights = () => {
        setMovieNights([]); // Clear state
        localStorage.removeItem('movieNights'); // Clear local storage
        alert('All planned movie nights have been cleared.');
    };
    ...
    return (
        <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
            ...
            <div>
                ...
                {movieNights.length > 0 && (
                    <button
                        onClick={handleClearAllMovieNights}
                        style={{
                            marginTop: '10px',
                            padding: '10px',
